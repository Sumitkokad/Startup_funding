...
def load_data():
    ...
    # Load data with the exact filenames as used originally
    ...
    # Stage mapping (same as original)
    stage_mapping = {
...
def get_yoy_growth(df, column='amount'):
...
def overall_analysis(df, original):
    ...
    st.markdown("#### Executive Summary of the Indian Startup Ecosystem")
    ...
    fig = px.bar(
        yoy_data, x='year', y='amount', text='growth',
    ...
    with col2:
        ...
        fig = px.bar(
            quarterly, x='year_quarter', y='amount',
    ...
    with col1:
        ...
        fig = px.bar(
            x=city_funding.values, y=city_funding.index, orientation='h',
    ...
    with col2:
        ...
        fig = px.treemap(
            sector_funding, path=['vertical'], values='amount',
    ...
    with col1:
        ...
        fig = px.bar(
            ...
            color=top_startups.values, color_continuous_scale='Greens'
        )
        fig.update_traces(texttemplate='%{text:,.0f}', textposition='outside')
        fig.update_layout(height=400, showlegend=False)
        st.plotly_chart(fig, use_container_width=True)
    ...
    with col2:
        ...
        fig = px.bar(
            x=top_investors.values, y=top_investors.index, orientation='h',
            text=top_investors.values,
            color=top_investors.values, color_continuous_scale='Oranges'
        )
        fig.update_traces(texttemplate='%{text:,.0f}', textposition='outside')
        fig.update_layout(height=400, showlegend=False)
        st.plotly_chart(fig, use_container_width=True)
...
def startup_analysis(df):
    ...
    with col1:
        ...
        fig = px.line(
            timeline, x='date', y='amount', markers=True,
    ...
    with col2:
        ...
        fig = px.bar(
            round_dist, x='Count', y='Round', orientation='h',
            text='Count',
...
def investor_analysis(df):
    ...
    with col1:
        ...
        fig = px.line(
            yearly, x='year', y='amount', markers=True,
    ...
    with col1:
        ...
        fig = px.treemap(
            sector, path=['Sector'], values='Count',
    ...
    with col2:
        ...
        fig = px.bar(
            stage, x='Count', y='Stage', orientation='h',
            text='Count',
    ...
    fig = px.bar(
        city, x='Count', y='City', orientation='h',
        text='Count',
...
def main():
    ...
    # Filter dataframe based on selections (applied to all dashboards)
    filtered_df = df[
